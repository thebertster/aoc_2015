...
for i in input:
    if i.startswith("turn"):
        ...
        break
    thru = i.find("through", oper+11)
    if thru >= 0:
        range1 = tuple(map(int,i[oper+7:thru].split(",")))
        range2 = tuple(map(int,i[thru+8:].split(",")))
    else:
        break
    mask = ((1 << (range2[0] - range1[0] + 1)) - 1) << range1[0]

    if oper == 0:
        for y in range(range1[1], range2[1]+1):
            bitArray[y] = bitArray[y] ^ mask
            totalBrightness += area*2
    elif oper == 1:
        for y in range(range1[1], range2[1]+1):
            bitArray[y] = (bitArray[y] & (allOnes ^ mask)) | mask
    elif oper == 2:
        for y in range(range1[1], range2[1]+1):
            bitArray[y] = bitArray[y] & (allOnes ^ mask)


countBits = 0

for bitField in bitArray:
    mask = 1
    for x in range(1000):
        if bitField & mask: countBits += 1
        mask <<= 1

print(countBits)
'''

lightArray = [x[:] for x in [[0] * 1000] * 1000]
totalBrightness = 0
