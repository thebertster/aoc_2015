...
while "a" not in outputs.keys():
    for i in input:
        arrow = i.find("->")
        if arrow>=0:
            wire = i[arrow+3:]
            if wire not in outputs.keys():
                oper = i[:arrow-1].split(" ")
                if len(oper) == 1:
                    v = GetValue(oper[0])
                    if v is not None: outputs[wire] = v
                elif len(oper) == 2 and oper[0] == "NOT":
                        v = GetValue(oper[1])
                        if v is not None: outputs[wire] = outputs[oper[1]] ^ 65535
                elif len(oper) == 3:
                    v1 = GetValue(oper[0])
                    v2 = GetValue(oper[2])
